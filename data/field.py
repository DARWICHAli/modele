...
class ImageDetectionsField(RawField):
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            ...
            print(f"DEBUG: precomp_data (raw): {precomp_data.shape}")
            print(f"DEBUG: cls_prob (raw): {f['%d_cls_prob' % image_id].shape} ->  {f['%d_cls_prob' % image_id][()].shape}")
        ...
        print(f"DEBUG: precomp_data (preprocessed): {precomp_data.astype(np.float32).shape}")
        return precomp_data.astype(np.float32)


class FasterImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(FasterImageDetectionsField, self).__init__(preprocessing, postprocessing)
...
class ImageDetectionsFieldWithBox(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(ImageDetectionsFieldWithBox, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32)


class ImageDetectionsFieldWithConcepts(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(ImageDetectionsFieldWithConcepts, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            f = h5py.File(self.detections_path, 'r')
            precomp_data = f['%d_features' % image_id][()]
            concepts = f['%d_concepts' % image_id][()]
        except KeyError:
            warnings.warn('Could not find detections for %d' % image_id)
            precomp_data = np.random.rand(10, 2048)
            concepts = np.random.rand(10, 4)
        ...
        return precomp_data.astype(np.float32), concepts.astype(np.float32)


class ImageAllFieldWithConcepts(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32), grid_data.astype(
            np.float32), concepts.astype(np.float32)


class ImageAllField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(ImageAllField, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        return precomp_data.astype(np.float32), relative_boxes.astype(np.float32), grid_data.astype(np.float32)
    ...
    def build_vocab(self, *args, **kwargs):
        ...
        specials = list(OrderedDict.fromkeys([
            tok for tok in [self.unk_token, self.pad_token, self.init_token,
                            self.eos_token]
            if tok is not None]))
        self.vocab = self.vocab_cls(counter, vectors='glove.6B.300d', specials=specials, **kwargs)
...
class UserImageDetectionsField(RawField):
    def __init__(self, features, proba_cls, max_detections=100,
                 sort_by_prob=False):
        self.max_detections = max_detections
        self.features = features
        self.proba_cls = proba_cls
        self.sort_by_prob = sort_by_prob
        self.cache = None

        super(UserImageDetectionsField, self).__init__(None, None)

    def preprocess(self, x, avoid_precomp=False):
        precomp_data = self.features[()]
        if self.sort_by_prob:
                precomp_data = precomp_data[np.argsort(np.max(self.proba_cls[()], -1))[::-1]]
